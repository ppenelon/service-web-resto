...
//Fonction qui génère un code client pour le système de fidélité
...
exports.genererCodeClient = function(idClient){
    ...
    lettre = alphabet.charAt(id % alphabet.length);
    ...
    lettre = alphabet.charAt(Math.floor(id / alphabet.length) % alphabet.length);
    chaineFinale = lettre + chaineFinale;
    //On ajoute un chiffre aléatoire de 0 à 9 à l'endroit _ (O-O_O-O)
    chaineFinale = Math.round(Math.random() * 9) + chaineFinale;
    //On calcule X (O-X-O-O)
    lettre = alphabet.charAt(Math.floor(id / (alphabet.length * alphabet.length)) % alphabet.length);
    chaineFinale = lettre + chaineFinale;
    //On ajoute un chiffre aléatoire de 0 à 9 à l'endroit _ (O_O-O-O)
    chaineFinale = Math.round(Math.random() * 9) + chaineFinale;
    //On calcule X (X-O-O-O)
    lettre = alphabet.charAt(Math.floor(id / (alphabet.length * alphabet.length * alphabet.length)) % alphabet.length);
    chaineFinale = lettre + chaineFinale;
